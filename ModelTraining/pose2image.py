...
def main(cfg):
    ...
    accelerator_config = ProjectConfiguration(
        project_dir=os.path.join(cfg.output_dir, cfg.exp_name),
    ...
    if accelerator.is_local_main_process:
        ...
        wandb_args = {"name": exp_name}
        if cfg.debug:
            wandb_args = {'mode':"disabled"}        
        accelerator.init_trackers(
            project_name="SSD", config=OmegaConf.to_container(cfg), init_kwargs={"wandb": wandb_args}
        )

        accelerator.project_configuration.project_dir = os.path.join(cfg.output_dir, wandb.run.name)
        accelerator.project_configuration.logging_dir = os.path.join(cfg.output_dir, wandb.run.name)
    else:
